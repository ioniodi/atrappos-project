...
import {deleteOnePath} from "../../../actions/pathsActions";
...
export const ConfirmDeletionModal = (props) => {
    ...
    const discarDrawnPath = () => {
        dispatch({
            ...
            edited: [],
            snapped: false,
            notificationToast: {show: true, type: 'success', msg: 'deletedDrawnPathToast'}
        });
        sendGaEvent({category: "discard-drawn-path", action: 'map-action'});
    }
    ...
    const defineModalAction = (type) => {
        switch (type) {
            ...
            case 'deleteDrawnPathModal':
                return discarDrawnPath();
